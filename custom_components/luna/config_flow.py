"""Config flow for Luna integration."""
...
from .const import DOMAIN, azimuth_step, elevation_step
        ...
        if user_input is not None:
            try:
                ...
                if az_step < 0.1 or el_step < 0.1:
                    ...
                    title = f"{DOMAIN.title()} - Lunar Position Sensors" if pending["location_name"] is None else f"{DOMAIN.title()} - {pending['location_name'].title()} - Lunar Position Sensors"
...
class LunaOptionsFlow(config_entries.OptionsFlow):
    """Handle Luna options."""
