...
LEGACY_NOTICE_NOTIFICATION_ID = "luna_legacy_notice_2026_05_01"
LEGACY_NOTICE_STORAGE_KEY = "luna_legacy_notice"
...
async def async_setup_entry(hass: HomeAssistant, entry: ConfigEntry) -> bool:
    ...
    # One-time legacy notice: show until dismissed; never re-show after user dismisses it.
    ...
    if not notice_data.get("dismissed", False):
        persistent_notification.create(
            hass,
            title="Luna: Final release — Breaking Sol update May 1, 2026",
            message=(
                "This is the final release of the current PyEphem-based Luna integration. "
                "Luna is discontinued and will no longer be maintained. "
                "Please migrate to Sol — the new Sol tracks the Sun, Moon, and planets "
                "in one integration: https://github.com/okkine/HA-Sol\n\n"
                ...
                "If you rely on Luna or Sol in production, please test the pre-release and update automations "
...
async def async_unload_entry(hass: HomeAssistant, entry: ConfigEntry) -> bool:
    ...
    remaining_entries = [
        key for key in hass.data[DOMAIN] if key != LEGACY_NOTICE_LISTENER_KEY
    ]
