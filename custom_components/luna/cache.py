"""Caching utilities for the Luna integration."""
...
from .utils import get_time_at_elevation
from .config_store import get_config_entry_data
from .const import DOMAIN

_LOGGER = logging.getLogger(__name__)

# Module-level cache for lunar events data - per entry instances
_lunar_events_cache_instances: Dict[str, Dict[str, Any]] = {}


class LunarEventsCache:
    """Manages caching for lunar transit/antitransit events."""
    ...
    def _get_entry_cache(self, entry_id: str) -> Dict[str, Any]:
        ...
        if entry_id not in _lunar_events_cache_instances:
            _lunar_events_cache_instances[entry_id] = {}
        return _lunar_events_cache_instances[entry_id]
    
    def _clear_entry_cache(self, entry_id: str):
        """Clear cache for a specific entry."""
        if entry_id in _lunar_events_cache_instances:
            del _lunar_events_cache_instances[entry_id]
    ...
    async def cleanup(self):
        ...
        _lunar_events_cache_instances.clear()
    ...
    async def _schedule_update_for_entry(self, entry_id: str):
        ...
        try:
            ...
            next_event_time, next_event_type, next_event_elevation, used_cache = self.get_cached_lunar_event(
                entry_id, None
            )
            ...
            if next_event_time:
                ...
                if delay > 0:
                    _LOGGER.debug(f"Scheduling lunar events cache update for {entry_id} in {delay:.2f} seconds at {update_time_local}")
                    ...
                    _LOGGER.warning(f"Next lunar event for {entry_id} is in the past, scheduling immediate update")
                    self._update_timers[entry_id] = self.hass.loop.call_later(
                        0.1,  # 100ms delay
                        lambda entry_id=entry_id: self.hass.async_create_task(
                            self._perform_scheduled_update_for_entry(entry_id)
                ...
                _LOGGER.warning(f"No lunar events found for {entry_id}, scheduling fallback update in 1 hour")
                self._update_timers[entry_id] = self.hass.loop.call_later(
                    3600,  # 1 hour
                    lambda entry_id=entry_id: self.hass.async_create_task(
                        self._perform_scheduled_update_for_entry(entry_id)
    ...
    async def _perform_scheduled_update_for_entry(self, entry_id: str):
        ...
        try:
            _LOGGER.debug(f"Performing scheduled lunar events cache update for {entry_id}")
            ...
            self.get_cached_lunar_event(entry_id, current_time)
            
            # Reschedule for this entry
            await self._schedule_update_for_entry(entry_id)
            
        except Exception as e:
            _LOGGER.error(f"Error during scheduled lunar events cache update for {entry_id}: {e}")
            # Schedule a retry in 5 minutes
            self._update_timers[entry_id] = self.hass.loop.call_later(
                300,  # 5 minutes
                lambda entry_id=entry_id: self.hass.async_create_task(
    ...
    async def add_entry(self, entry_id: str):
        ...
        # Entry will be automatically scheduled when get_cached_lunar_event is first called
        pass
    ...
    def _calculate_next_lunar_event(self, entry_id: str, current_time_local: datetime.datetime) -> Tuple[Optional[datetime.datetime], Optional[str], Optional[float]]:
        """
        Calculate the next lunar event (transit/antitransit) and its elevation.
        ...
        """
        from .utils import get_moon_position
        from .config_store import get_config_entry_data
        
        # Get config data
        config_data = get_config_entry_data(entry_id)
        
        # Get current moon position data to find next lunar events
        moon_data = get_moon_position(self.hass, current_time_local, entry_id, config_data=config_data)
        
        lunar_transit_dt = moon_data.get('lunar_transit')
        lunar_antitransit_dt = moon_data.get('lunar_antitransit')
        
        # Determine which event is next
        next_event_time = None
        next_event_type = None
        
        if lunar_transit_dt and lunar_antitransit_dt:
            if lunar_transit_dt < lunar_antitransit_dt:
                next_event_time = lunar_transit_dt
                next_event_type = 'transit'
            else:
                next_event_time = lunar_antitransit_dt
                next_event_type = 'antitransit'
        elif lunar_transit_dt:
            next_event_time = lunar_transit_dt
            next_event_type = 'transit'
        elif lunar_antitransit_dt:
            next_event_time = lunar_antitransit_dt
            next_event_type = 'antitransit'
        
        # Calculate the exact elevation at the lunar event time
        next_event_elevation = None
        if next_event_time:
            try:
                # Convert UTC time back to local time for get_moon_position
                local_tz = zoneinfo.ZoneInfo(self.hass.config.time_zone)
                next_event_local = next_event_time.astimezone(local_tz)
                
                # Get moon position at the event time
                event_moon_data = get_moon_position(self.hass, next_event_local, entry_id, config_data=config_data)
                next_event_elevation = event_moon_data.get('elevation')
            except Exception:
                next_event_elevation = None
    ...
    def get_cached_lunar_event(
        self, 
        entry_id: str,
        current_time: Optional[datetime.datetime] = None
    ) -> Tuple[Optional[datetime.datetime], Optional[str], Optional[float], bool]:
        """
        Get cached lunar event data or calculate and cache if needed.
        ...
        if entry_cache and 'calculated_at' in entry_cache:
            ...
            if calculated_at and next_event_time:
                ...
                if next_event_time <= current_time_utc:
                    _LOGGER.debug(f"Cached lunar event for {entry_id} is in the past, refreshing cache")
    ...
    def _initialize_cache(
        ...
        next_event_time, next_event_type, next_event_elevation = self._calculate_next_lunar_event(entry_id, current_time)
        ...
        if next_event_time:
            entry_cache.update({
                'next_event_time': next_event_time,  # UTC time from get_moon_position
            ...
            self.hass.bus.async_fire(
                f"{DOMAIN}_lunar_events_cache_updated",
                {
                    "entry_id": entry_id,
...
# Global lunar events cache instance
_lunar_events_cache_instance: Optional[LunarEventsCache] = None

def get_lunar_events_cache_instance(hass: HomeAssistant) -> LunarEventsCache:
    """Get the global lunar events cache instance."""
    global _lunar_events_cache_instance
    if _lunar_events_cache_instance is None:
        _lunar_events_cache_instance = LunarEventsCache(hass)
    return _lunar_events_cache_instance

# Convenience function for lunar events cache
def get_cached_lunar_event(
    hass: HomeAssistant,
    entry_id: str,
    current_time: Optional[datetime.datetime] = None
    ...
    """
    Convenience function to get cached lunar event data.
    ...
    Returns:
        Tuple (next_event_time, next_event_type, next_event_elevation, used_cache)
    """
    cache = get_lunar_events_cache_instance(hass)
    return cache.get_cached_lunar_event(entry_id, current_time) 